---
title: "26.10 - Kleine Übernahme"
date: "2026-03-24T16:03:50.295Z"
category: "sneakpeeks"
excerpt: "Bereit oder nicht, hier kommt der Tiny Takeover Drop! Neue Versionen von Baby-Mobs haben Minecraft gestürmt, wodurch niedliche Überlastung über Biomes und baut. Hören Sie auf für herzschmelzende Klänge, Craft Name Tags für Ihre Haustiere, und halten Babys für immer jung mit th"
cover: "https://launchercontent.mojang.com/v2/images/26.10.png"
tags: ["update", "26.10"]
_generated: true
_source: "https://launchercontent.mojang.com/v2/bedrockPatchNotes/a85e4952d3b347bbdf41dd131e838a6adfe4a3226a94aebcf71339a50b69ba81.json"
---
...
-   Added "held\_item\_scale" field to entity.json schema
...
-   Replaced the "simulate\_waves" field of the "minecraft:buoyant" component with the new "movement\_type" field
    
    -   The "movement\_type" field accepts one of three values:
    ...
    -   The "big\_wave\_probability" and "big\_wave\_speed" fields are only used when "movement\_type" is set to "waves"
-   Added a "can\_auto\_step\_from\_liquid" field to the "minecraft:buoyant" component
...
-   Added the "is\_tamed" entity filter, telling if the subject entity is tamed ([MCPE-234672](https://bugs.mojang.com/browse/MCPE-234672))
...
-   The colored block lighting feature is no longer restricted to the Render Dragon Features for Creator Experiment
    ...
    -   To enable this feature, PBR resource packs must include a local\_lighting.json with `static_light` entries. Blocks that do not have a `static_light` and `light_color` specified via a provided resource pack will be treated as non-colored lighting and use the standard uniform light color.
...
-   Removed ESSL\_300 shader language support
...
-   The minecraft:block\_placer item component has a new boolean field "aligned\_placement" ([MCPE-234686](https://bugs.mojang.com/browse/MCPE-234686))
...
-   Blocks can handle the following entity events in scripting through the "onEntity" handler:
    ...
    -   Custom named events:
        -   "on\_home" from "minecraft:behavior.go\_home"
        -   “on\_escape” from "minecraft:behavior.avoid\_block"
        -   "on\_reach" from "minecraft:behavior.move\_to\_block"
        -   "on\_place" from "minecraft:behavior.place\_block"
        -   "on\_take" from "minecraft:behavior.take\_block"
...
-   It defines how a block placed above a chest or ender\_chest should obstruct their opening
    
    -   "obstruction\_rule" (Optional): Specifies how the block is evaluated during chest opening if placed above; "shape" is the default when not specified.
