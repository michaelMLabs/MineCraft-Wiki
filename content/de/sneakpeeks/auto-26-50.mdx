---
title: "26.50 - Wilderness Bound"
date: "2026-09-15"
category: "sneakpeeks"
excerpt: "Wilderness Bound ist jetzt in Minecraft! Packen Sie Ihre Vorräte und setzen Sie auf eine Expedition in den entführten Wald, wo die Trails enden und Abenteuer beginnt. Seek aus verlassenen Lagern, bauen Außenposten unter den Pappelbäumen, schlafen unter den Sternen auf Strohbett"
cover: "https://launchercontent.mojang.com/v2/images/26.50.jpg"
tags: ["update", "26.50"]
_generated: true
_source: "https://launchercontent.mojang.com/v2/bedrockPatchNotes/f97d0b95c0ba1acf9d8e41bcd251320831caf6d8c0a14f0f5dbf0f124c05bcb1.json"
---
...
-   Straw Beds can be found in Abandoned Camps
-   Cushions can be found in Abandoned Camps
-   Chests in Abandoned Camps contain Explorer Maps that lead to Ancient City, Mineshaft, Desert Pyramid, Warm Ocean Ruins, and other biome variants of Abandoned Camps

## Blocks

-   Added Wool Stairs and Wool Slab blocks in all 16 colors
-   Added Concrete Stairs and Concrete Slab blocks in all 16 colors
...
-   Straw Beds breaks faster when using a Hoe
...
-   Fixed block connection states being lost when loading a world saved before those states existed, which could leave fences, stairs, thin fences, and trip wire drawn as if they were not connected
...
-   Reduced main thread work during world load by removing the redundant precaching of the world settings screen
...
-   PS4/PS5: GUI Scale modifier will now be respected on title launch ([MCPE-89543](https://bugs.mojang.com/browse/MCPE-89543))
...
-   Made the 'minecraft:anger\_level' schema stricter when parsing and fail to load an entity json that has invalid data in versions 1.26.50 and newer.
...
-   Made schema for `minecraft:projectile` stricter when parsing and will fail to load an entity json that has invalid data in versions 1.26.50 and newer.
    ...
    -   `anchor` field now takes the following string values instead of integers: "origin", "eye\_height", "middle"
...
-   Updated "minecraft:behavior.follow\_mob" : whenever the goal stops or is removed through a component group change, the mob's navigation is always interrupted.
...
-    Released `class EntityTamedEventOptions`from `beta` to `v2.10.0`
...
-   Added a new `multiselect` pack setting type, letting pack authors declare a list of options   - Molang can query for active options in the multiselect via `query.is_pack_setting_selected`   - `world.getPackSettings()` now can return an array of strings for multiselect options in `@minecraft/server` beta   - `PackSettingsChangeAfterEvent.settingValue` now can be an array of strings for multiselect options in `@minecraft/server` beta
...
-   Added version 2 of the structure (`.mcstructure`) file format
    -   The `block_indices` arrays are now stored as NBT `IntArrayTag`s instead of a `ListTag` containing one `IntTag` per block, which significantly reduces the time and memory needed to save and load structures
...
-   Camera presets that inherit from minecraft:follow\_orbit no longer reset the player's rotation the first time they are applied, for presets using format version 1.26.50 or later
...
-   Entities with the following runtime\_identifiers will be upgraded to automatically have this component:
-   Vulnerability to freezing damage from Powder Snow is no longer hardcoded at runtime and is instead defined by the `minecraft:freezing_vulnerable` component     - Entities with the following runtime\_identifiers will be upgraded to automatically have this component:         - "minecraft:blaze"         - "minecraft:magma\_cube"         - "minecraft:strider"         "minecraft:freezing\_vulnerable": {}
...
-   Fixed MERS not applying to assets in multiple Vanilla content slices
    ...
    -   1.26.20 Baby Bee
    -   1.26.20 Baby Fox
-   Fixed .texture\_set.json not being compatible with certain JSON tools
...
-   Changed class `MessageBox`
    -   Added function `button1WithOptions`
        ...
        ```ts
        button1WithOptions(label: ObservableString | ObservableUIRawMessage | 
        string | UIRawMessage, options?: MessageBoxButtonOptions): MessageBox
        ```
        
    -   Added function `button2WithOptions`
        
        ```ts
        button2WithOptions(label: ObservableString | ObservableUIRawMessage | 
        string | UIRawMessage, options?: MessageBoxButtonOptions): MessageBox
        ```
...
-   Fixed wrapping for labels and headers when using large texts with no spaces between. \[JIRA MCPE-241081\] Fix for players when leaving world while having a DDUI screen is open, becoming soft-locked when re-joining.
